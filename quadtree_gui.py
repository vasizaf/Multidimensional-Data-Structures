...
class QuadtreeGUI:
    ...
    def update_conditions(self, selected_col):
        ...
        # Show condition entry fields for selected attributes
        for col in self.available_columns:
            if col in self.selected_attributes:
                self.cond_labels[col].grid()  # Show the label
                if col in self.numeric_attributes:
                    # Show "Min" and "Max" labels and entry fields for numeric attributes
                    self.min_entries[col].grid()  # Min entry
                    self.max_entries[col].grid()  # Max entry
                else:
                    self.single_entries[col].grid()  # Show the single entry
            else:
                self.cond_labels[col].grid_remove()  # Hide the label
                if col in self.numeric_attributes:
                    # Hide "Min" and "Max" labels and entry fields for numeric attributes
                    self.min_entries[col].grid_remove()  # Min entry
                    self.max_entries[col].grid_remove()  # Max entry
                else:
                    self.single_entries[col].grid_remove()  # Hide the single entry
    ...
    def perform_search(self):
        ...
        # Call the Quadtree main function with selected attributes, conditions, review keywords, and num_neighbors
